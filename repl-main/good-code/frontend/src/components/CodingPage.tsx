...
function useSocket(replId: string) {
  ...
  useEffect(() => {
    const newSocket = io(`ws://${replId}.kesartechnologies.software`);
    setSocket(newSocket);

    newSocket.on("disconnect", () => {
      console.log("Socket disconnected, stopping pod after delay...");

      setTimeout(() => {
        axios
          .post("http://localhost:3002/stoppod", { replId })
          .then(() => {
            console.log("Pod stopped successfully");
          })
          .catch((error) => {
            console.error("Error while stopping the pod", error);
          });
      }, 1000); // 3 minutes delay
    });
...
export const CodingPagePostPodCreation = () => {
  ...
  const opennewtab = () => {
    const INSTANCE_URI = `http://${replId}.rudrkesar.engineer`;
    window.open(INSTANCE_URI, "_blank");
  };
  return (
    <Container>
      <ButtonContainer>
